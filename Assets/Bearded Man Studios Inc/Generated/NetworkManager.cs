...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] CubeTestNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is CubeTestNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (CubeTestNetworkObject.Length > 0 && CubeTestNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(CubeTestNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<CubeTestBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateCubeTest instead, its shorter and easier to type out ;)")]
		public CubeTestBehavior InstantiateCubeTestNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(CubeTestNetworkObject[index]);
			var netBehavior = go.GetComponent<CubeTestBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<CubeTestBehavior>().networkObject = (CubeTestNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of CubeTest
		/// </summary>
		/// <returns>
		/// A local instance of CubeTestBehavior
		/// </returns>
		/// <param name="index">The index of the CubeTest prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public CubeTestBehavior InstantiateCubeTest(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(CubeTestNetworkObject[index]);
			var netBehavior = go.GetComponent<CubeTestBehavior>();
